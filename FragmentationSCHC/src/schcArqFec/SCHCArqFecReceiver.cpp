...
SCHCArqFecReceiver::SCHCArqFecReceiver(SCHCFragDir dir, AppConfig &appConfig, SCHCCore &schcCore, SCHCSession &schcSession): _dir(dir), _appConfig(appConfig), _schcCore(schcCore), _schcSession(schcSession)
{
...
SCHCArqFecReceiver::~SCHCArqFecReceiver()
{
    SPDLOG_DEBUG("Executing SCHCArqFecReceiver destructor()");
}

void SCHCArqFecReceiver::execute(const std::vector<uint8_t>& msg)
{
    ...
    {
        if(_nextStateStr == SCHCArqFecReceiverStates::STATE_INIT)
        {
            SPDLOG_DEBUG("Changing STATE to STATE_INIT");
            _currentState = std::make_unique<SCHCArqFecReceiver_INIT>(*this);
            _currentStateStr = SCHCArqFecReceiverStates::STATE_INIT;
        }
        else if (_nextStateStr == SCHCArqFecReceiverStates::STATE_RCV_WINDOW)
        {
            SPDLOG_DEBUG("Changing STATE to STATE_RCV_WINDOW");
            _currentState = std::make_unique<SCHCArqFecReceiver_RCV_WINDOW>(*this);
            _currentStateStr = SCHCArqFecReceiverStates::STATE_RCV_WINDOW;
        }
        else if (_nextStateStr == SCHCArqFecReceiverStates::STATE_WAIT_X_ALL1)
        {
            SPDLOG_DEBUG("Changing STATE to STATE_WAIT_X_ALL1");
            _currentState = std::make_unique<SCHCArqFecReceiver_WAIT_X_ALL_1>(*this);
            _currentStateStr = SCHCArqFecReceiverStates::STATE_WAIT_X_ALL1;
        }
...
void SCHCArqFecReceiver::release()
{
    SPDLOG_DEBUG("Releasing the memory of the current state");
    _currentState.reset();
}

void SCHCArqFecReceiver::executeAgain()
{
...
void SCHCArqFecReceiver::executeTimer(int delay)
{
    _timer.start(std::chrono::seconds(delay), [&]() { enqueueTimer(); });
}

void SCHCArqFecReceiver::enqueueTimer()
{
