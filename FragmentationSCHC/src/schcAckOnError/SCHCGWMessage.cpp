...
SCHCGWMessage::SCHCGWMessage()
{
}

std::vector<uint8_t> SCHCGWMessage::create_schc_ack(uint8_t rule_id, uint8_t dtag, uint8_t w, uint8_t c, std::vector<uint8_t> bitmap_vector, bool must_compress)
{
...
std::vector<uint8_t> SCHCGWMessage::create_schc_ack_compound(uint8_t rule_id, uint8_t dtag, int last_win, const std::vector<uint8_t> c_vector, const std::vector<std::vector<uint8_t>> bitmap_array, uint8_t win_size)
{
    std::vector<uint8_t> buffer;
...
SCHCMsgType SCHCGWMessage::get_msg_type(ProtocolType protocol, uint8_t rule_id, std::vector<uint8_t> msg)
{
    ...
    if(protocol==ProtocolType::LORAWAN_NS || protocol==ProtocolType::MYRIOTA_NS)
    {
...
uint8_t SCHCGWMessage::decode_message(ProtocolType protocol, uint8_t rule_id, std::vector<uint8_t> msg)
{
    if(protocol==ProtocolType::LORAWAN_NS || protocol==ProtocolType::MYRIOTA_NS)
    {
        int len = msg.size();
...
uint8_t SCHCGWMessage::get_w()
{
    return _w;
}

uint8_t SCHCGWMessage::get_fcn()
{
    return _fcn;
}

uint8_t SCHCGWMessage::get_dtag()
{
    return _dtag;
}

int SCHCGWMessage::get_schc_payload_len()
{
    return _schc_payload_len;
}

std::vector<uint8_t> SCHCGWMessage::get_schc_payload()
{
    return _schc_payload;
}

uint32_t SCHCGWMessage::get_rcs()
{
    return _rcs;
}

std::string SCHCGWMessage::get_compound_bitmap_str()
{
    return _compound_ack_string;
}

void SCHCGWMessage::printMsg(ProtocolType protocol, SCHCMsgType msgType, std::vector<uint8_t> msg)
{
    int len = msg.size();
    char buff[100];
