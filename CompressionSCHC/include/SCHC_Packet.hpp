...
This header file defines the structures and enumerations necessary for representing SCHC 
(Static Context Header Compression) packets and helpers to write its content.
*/
...
#include <vector>
#include <stdexcept>
#include <type_traits> //to veryfy that T is an unsigned integer in write_uint

//+------------------------------------------------------------------------------------------------------+

//Helper class to write bit by bit value into a byte buffer
//Used to generate the residue and the compressed SCHC packet
class BitWriter {
    private:
        std::vector<uint8_t> buffer;
        uint32_t bitlen = 0; //bits already written in the buffer
...
//Class representing a SCHC Compressed Packet, cointaining the selected rule, the compression residue, the payload 
//of the original packet and the raw packet generated by the compression process.
